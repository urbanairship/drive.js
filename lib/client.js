...
proto.bundle_browserify = function(tests, ready) {
  ...
  function onfound(err, requirements) {
    ...
    Object.keys(requirements).forEach(process_requirements)

    bundle_tests(function() {
      console.time('base_bundling')
    ...
    function bundle_tests(ready) {
      (function bundle() {
        var file = tests.shift()
          , test_bundler = browserify(browserify_options)

        console.time(file + '_bundling')
        transforms.forEach(add_transform.bind(null, test_bundler))
        test_bundler.transform(require('./test_transform'))
        Object.keys(requirements).forEach(function(el) {
          test_bundler.external(requirements[el].full)
        })
        externals.forEach(function (el) {
          test_bundler.ignore(resolve_file(el))
        })
        test_bundler.add(path.resolve(process.cwd(), file))
        ...
        function write_test(err, data) {
          ...
          console.timeEnd(file + '_bundling')

          return tests.length ? bundle() : ready()
        }
      })()
  ...
  function add_transform(bundler, file) {
  ...
  function onbundled(err, data) {
    ...
    console.timeEnd('base_bundling')
    console.timeEnd('bundling')
    ready(err, data, test_bundles)
  }
