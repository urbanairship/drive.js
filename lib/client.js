...
proto.update = function(file, msg, suite, test_count, test_num, test_name) {
  this.dnode.update(file, msg, suite, test_count, test_num, test_name)
}
...
proto.update = function(name, type, suite, test_count, test_num, test_name) {
  var msg

  if(this.cli.options.tap) {
    if(test_num === 1 && test_count) {
      this.cli.write('1..' + test_count + '\n')
      this.cli.write('# ' + suite + '\n')
    }

    if(!test_count) {
      this.cli.write('1..1\n')
      this.cli.write('# *ERROR* ' + suite + '\n')
    }

    msg = type === 'pass' ? 'ok ' : 'not ok '
    msg += test_num + ' - ' + test_name + '\n'
  } else {
    msg = {
        pass: '.'
      , fail: 'F'
