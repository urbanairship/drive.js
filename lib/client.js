...
function Client(dnode, conn, for_browserify, browserify_options) {
...
proto.bundle_browserify = function(tests, ready) {
  var self = this
    ...
    , base_bundler_options = {}
    , test_bundler_options = {}
    , browserify_options = {}
    , CWD = process.cwd()
    , test_bundles = {}
  ...
  base_bundler_options.insertGlobals = true
  base_bundler_options.debug = true
  test_bundler_options.insertGlobals = true
  test_bundler_options.debug = true
  ...
  var base_bundler = browserify(browserify_options)

  find_requirements(tests.slice(), onfound)

  function onfound(err, requirements) {
    var requirement_keys = Object.keys(requirements)

    externals.forEach(function(el) {
      base_bundler.external(resolve_file(el))
    })
    transforms = transforms.map(resolve_transform)
    transforms.forEach(add_transform.bind(null, base_bundler))

    reqs.forEach(add_requires)
    requirement_keys.forEach(process_requirements)

    console.time('bundling')
    bundle_tests(function() {
     base_bundler.bundle(base_bundler_options, onbundled)
    })
    ...
    function onbundled(err, data) {
      ...
      console.endTime('bundling')

      ready(null, data, test_bundles)
    }

    function process_requirements(el) {
      base_bundler.require(
          requirements[el].full
        , { expose: el }
      )
    }

    function test_bundler() {
      var test_transform = require('./test_transform')

      var all_externals = requirement_keys.concat(externals)

      return do_bundle

      function do_bundle(file, ready) {
        var test_bundler = browserify(browserify_options)

        transforms.forEach(function(el) {
          test_bundler.transform(el)
        })
        test_bundler.transform(test_transform)
        all_externals.forEach(function(el) {
          test_bundler.ignore(resolve_file(el))
        })
        test_bundler.add(path.resolve(CWD, file))
        test_bundler.bundle(test_bundler_options, write_test)

        function write_test(err, data) {
          if(err) {
            return ready(err)
          }
    ...
    function bundle_tests(ready) {
      ...
      function do_bundle() {
        ...
        function write_test(err, data) {
          ...
          test_bundles[file] = data

          return tests.length ? do_bundle() : ready()
        }
      }
    }
  }

  function resolve_transform(file) {
    try {
      return require(resolve_file(file))
    } catch(e) {
      return require('through')
    }
  }

  function add_requires(file) {
    base_bundler.require(file)
  }

  function add_transform(bundler, tr) {
