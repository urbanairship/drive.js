    ...
    function bundle_tests(ready) {
      (function bundle() {
        var file = tests.shift()
          , test_bundler = browserify({ noParse: (browserify_options.noParse || []) })//{ noParse: [path.resolve(process.cwd(), file)] })

        transforms.forEach(add_transform.bind(null, test_bundler))
        test_bundler.transform(require('./test_transform'))
        //test_bundler.external(base_bundler)
        if (browserify_options.noParse) {
          browserify_options.noParse.forEach(function (el) {
            test_bundler.ignore(el)
          })
        }
        Object.keys(requirements).forEach(function(el) {
          test_bundler.external(el)
        })
