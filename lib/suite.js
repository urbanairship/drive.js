...
proto.respond_media = function(req, resp, endpoint, env) {
  ...
  self.request.client.load_media(endpoint, function(err, data) {
    ...
    if(mimetype === 'text/javascript' || mimetype === 'application/javascript') {
      if (self.request.browserify) {
        data = self.rewrite_for_browserify(data, self.request.tests)
      }
...
proto.rewrite_for_browserify = function(data, tests) {
  ...
  function onparsed(err, requirements) {
    ...
    return falafel(data, process_function)
