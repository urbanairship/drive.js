...
proto.respond_media = function(req, resp, endpoint, env) {
  ...
  self.request.client.load_media(endpoint, function(err, data) {
    ...
    if(mimetype === 'text/javascript' || mimetype === 'application/javascript') {
      if (self.request.client.browserify || (self.request.client.dnode && self.request.client.dnode.browserify)) {
...
proto.rewrite_for_browserify = function(data, tests, ready) {
  ...
  function onparsed(err, requirements) {
    var is_require = lang('function call id[name=require]:first-child + literal')
      , is_require_pass = lang('call > *:first-child ~ * > id[name=require]')
    return ready(null, '' + falafel(data, process_function))
    function process_function(node) {
      if (is_require(node)) {
        var lookup = node.source().replace(/[\'|\"]/g, '')
        requirements[lookup] && node.update('\'' + requirements[lookup].named + '\'')
      }
