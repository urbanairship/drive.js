...
proto.respond_media = function(req, resp, endpoint, env, run) {
  ...
  is_js = mimetype == 'text/javascript' || mimetype == 'application/javascript'

  if(client.browserify && is_js) {
    return media_loaded(null, run.test_bundles[endpoint])
    //return run.test_bundler(path.resolve(process.cwd(), endpoint), media_loaded)
  }
