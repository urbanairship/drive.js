...
proto.build_browserify = function(cli_reqs, deps, transforms, no_parse, test_files, uuid, ready) {
  var self = this
  ...
  if (deps.length) {
    browserify_options.entries = deps.map(path.resolve.bind(path, CWD))
  }
  if (no_parse.length) {
    browserify_options.noParse = no_parse.map(path.resolve.bind(path, CWD))
  }
  ...
  function respond_new_test() {
    ...
    if (test_req.client.browserify || (test_req.client.dnode && test_req.client.dnode.browserify)) {
      ...
      var transforms = client.browserify_transforms
        , no_parse = client.browserify_noparse
        , deps = client.browserify_deps
        , reqs = client.browserify_requires

      self.build_browserify(reqs, deps, transforms, no_parse, test_req.tests.slice(), test_run.uuid, continue_request.bind(self))
    } else {
