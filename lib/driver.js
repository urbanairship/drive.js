...
var find_requirements = require('./find_requirements')
  , Environment = require('./environment')        // <-- a browser (or node) providing an environment
...
proto.browserify = function(reqs, deps, transforms, noparse, tests, id, ready) {
  var self = this
    , run = self.get_run_by_uuid(id)
    , browserify_options = {}
    , CWD = process.cwd()

  browserify_options.insertGlobals = true
  browserify_options.debug = true
  ...
  find_requirements(tests, onprocessed)

  function resolve_file(file) {
    return path.resolve(CWD, file)
  }

  function onprocessed(err, requirements) {
    ...
    function process_requirements(el) {
      b.require(requirements[el].full, { expose: requirements[el].named })
    }
    ...
    function write_bundle(err, data) {
      ...
      self.browserify_bundles[id] = data
      ready(null)
    }
  ...
  function respond_new_test() {
    ...
    if(test_req.client.browserify || (test_req.client.dnode && test_req.client.dnode.browserify)) {
      if(test_req.client.dnode) {
        var client = test_req.client.dnode
      } else {
        var client = test_req.client
      }

      var transforms = client.browserify_transforms
        , noparse = client.browserify_noparse
        , reqs = client.browserify_requires
        , deps = client.browserify_deps
