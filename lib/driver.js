...
proto.get_bundled_js = function(ready) {
  ...
  function done() {
    output = [
      ...
      , output.join('</script><script type="text/javascript">')
      , '</script>'
      , base_bundle
    ].join('')
...
proto.request_tests = function(drive_client, client, tests, wants, want_repl, ident) {
  var self = this
    , request = new Request(ident, drive_client, tests, wants || [], want_repl)
    , missing
