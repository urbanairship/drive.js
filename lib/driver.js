...
proto.get_bundled_js = function(for_browserify, ready) {
  ...
  if(self.bundle_cache)
    return ready(null, self.bundle_cache + '<script type="text/javascript" src="/media/' + (for_browserify ? 'browserify-' : '') + 'suite.js"></script>')
  ...
  function nab_bundle(filename, idx) {
    if (for_browserify && /behavior/.test(filename)) {
      fs.createReadStream(filename).pipe(require('./transform-amd')({})()).pipe(concat(parse_file.bind(null, null)))
    } else {
  ...
  function done() {
    ...
    ready(null, self.bundle_cache + '<script type="text/javascript" src="/media/' + (for_browserify ? 'browserify-' : '') + 'suite.js"></script>') 
  }
}
proto.build_browserify = function (test_files, uuid, ready) {
  var b = browserify({ insertGlobals: true, debug: true, noParse: ['/Users/jessekeane/vagrant/airship/airship/airship/static/js/thirdparty/bops.js'] })
    , self = this
  ...
  function onprocessed(err, reqs) {
    ...
    b.transform(require('./transform-amd')({}))
    b.transform(require('./transform-plate'))
    b.bundle().pipe(concat(write_bundle))
    function process_requirements(el) {
      b.require(reqs[el].full, { expose: reqs[el].named })
    }
    function write_bundle(data) {
...
proto.xhr_idle = function(req, resp, uuid) {
  ...
  function respond_new_test() {
    ...
    if (test_req.client.browserify) {
      self.build_browserify(test_req.tests.slice(), test_run.uuid, continue_request.bind(self))
    } else {
...
proto.env_suite = function(req, resp, env_uuid, run_uuid) {
  var self = this
    ...
    , is_browserify = (run.request.client.for_browserify() || (run.request.client.dnode && run.request.client.dnode.for_browserify()))
