...
proto.build_browserify = function (transforms, no_parse, test_files, uuid, ready) {
  var b = browserify({ insertGlobals: true, debug: true, noParse: no_parse })
    , self = this
  if (self.configuration && self.configuration.browserify && self.configuration.browserify.deps && self.configuration.browserify.deps.length) {
    self.configuration.browserify.deps.forEach(add_deps)
  }

  find_requirements(test_files, onprocessed)
  function add_deps(file) {
    b.add(path.resolve(process.cwd(), file))
  }
