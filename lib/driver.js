...
proto.get_bundled_js = function(ready) {
  ...
  function done() {
    output = '<script type="text/javascript" src="/media/redirect.js"></script>' + output.join('') + base_bundle

    ready(null, self.bundle_cache = output) 
  }
}
proto.build_browserify = function (test_files, uuid, ready) {
  console.log('building a browserify bundle!')
  var b = browserify()
    , self = this
  ...
  function onprocessed(err, reqs) {
    ...
    function process_requirements(el) {
      b.require(el, { expose: reqs[el] })
    }
    function write_bundle(data) {
      self.browserify_bundle[uuid] = data
      ready(null)
    }
  }
}
...
proto.request_tests = function(drive_client, client, tests, wants, want_repl, want_coverage, ident) {
  ...
  if (drive_client.browserify || drive_client.dnode.browserify) {
    self.build_browserify(tests, request.run.uuid, continue_test)
  } else {
    continue_test()
  }
  function continue_test() {
    var request = new Request(ident, drive_client, tests, wants || [], want_repl, want_coverage)
      , missing
...
proto.env_suite = function(req, resp, env_uuid, run_uuid) {
  ...
  if (run.request.client.browserify && !self.browserify_bundles[run_uuid]) {
    self.build_browserify(run.tests, run_uuid, then_bundle)
  } else {
    then_bundle()
  }
  ...
  function then_load(err, bundled_js) {
    ...
    run.request.client.load(run.current().name, function(err, data) {
      ...
      suite.get_html(function got_suite_html(err, html) {
        ...
        if (!run.request.client.browserify) {
