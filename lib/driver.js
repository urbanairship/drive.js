...
function Driver(port, bundle, browserify) {
  this.port = port
...
proto.build_browserify = function (test_files, ready) {
  var b = browserify()
    , self = this
...
proto.request_tests = function(drive_client, client, tests, wants, want_repl, want_coverage, ident) {
  ...
  if (this.browserify) {
    this.build_browserify(tests, continue_request.bind(this))
  } else {
    continue_request().bind(this)
  }
...
proto.browserify_bundle = function (req, resp) {
  if (this.browserify && this.browserify_data) {
    resp.writeHead(200, {'Content-Type':'text/javascript'})
    resp.end(this.browserify_data)
  } else {
    resp.writeHead(404, {'Content-Type':'text/plain'})
    resp.end('No browserify bundle')
  }
}
  ...
  function then_load(err, bundled_js) {
    ...
    run.request.client.load(run.current().name, function(err, data) {
      ...
      suite.get_html(function got_suite_html(err, html) {
        ...
        if (!self.browserify_data) {
          bundled_js += '<script type="text/javascript" src="/media/3p/require.min.js" data-main="'+main_url+'"></script>'
        } else {
          bundled_js += '<script type="text/javascript" src="/media/browserify/' + env_uuid + '.js"></script>'
        }
