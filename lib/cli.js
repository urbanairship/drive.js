...
function CLI(argv, streams) {
  streams = streams || {}
  this.options = nopt(this.noptions, this.shorthands, argv)
...
proto.client = function(driver_url) {
  ...
  var parsed = url.parse(!isNaN(driver_url) ? 'http://localhost:'+driver_url+'/' : driver_url)
    , rest = this.get_tests()
    , client = new Client(this, rest, this.options.browserify, this.options.transform, this.options.noparse, this.options.entry, this.options.require)
...
proto.local = function() {
  ...
  function got_env() {
    var client = new Client(self, self.options.argv.remain, self.options.browserify, self.options.transform, self.options.noparse, self.options.entry, self.options.require)
      , rest = self.get_tests()
