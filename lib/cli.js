...
proto.get_config = function () {
  var config_file = path.join(process.cwd(), '.drive.json')
  try {
...
proto.spawn = function(port) {
  ...
  var driver = new Driver(null, this.options.bundle, this.get_config())
...
proto.client = function(driver_url) {
  ...
  var parsed = url.parse(!isNaN(driver_url) ? 'http://localhost:'+driver_url+'/' : driver_url)
    , rest = this.get_tests()
    , client = new Client(this, rest, this.options.browserify, this.options.transform, this.options.noparse, this.options.dep)
...
proto.local = function() {
  ...
  var self = this
    , port = 9000
    , driver = new Driver(null, self.options.bundle, self.get_config())
  ...
  function got_env() {
    var client = new Client(self, self.options.argv.remain, self.options.browserify, self.options.transform, self.options.noparse, self.options.dep)
      , rest = self.get_tests()
